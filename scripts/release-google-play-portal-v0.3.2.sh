...
OUTPUT_PATH="${1:-CareSuite-HealthOS-0.3.2.aab}"
...
deploy_release_database_migration() {
  ...
  printf 'Produktive Migrationshistorie wird in einen isolierten Release-Arbeitsbereich geladen ...\n'
  (
    cd "$RELEASE_DB_ROOT"
  ...
  [[ "$local_only_versions" = '20260902103000' ]] \
    || die "Unerwartete lokale Release-Migrationen: ${local_only_versions:-keine}"

  printf 'Isolierter Dry-Run für ausschließlich Migration 20260902103000 ...\n'
  (
    cd "$RELEASE_DB_ROOT"
    npx --yes supabase@latest db push --linked --dry-run
  )

  printf 'Migration 20260902103000 wird produktiv ausgerollt ...\n'
  (
    cd "$RELEASE_DB_ROOT"
...
if (app.version !== '0.3.2') throw new Error('Falsche Nutzer-Version.');
if (app.android?.versionCode < 25) throw new Error('Falsche lokale versionCode-Baseline.');
...
npx vitest run \
  src/__tests__/platform/googlePlayUpdateV0_3_2.test.ts \
  src/__tests__/platform/googlePlayMigrationIsolationR20_3.test.ts \
...
printf 'Portal-Login und Sitzungsreparatur werden als zusammengehöriges Backend ausgerollt ...\n'
...
printf '\nGoogle-Play-Update 0.3.2 wurde vollständig gebaut und heruntergeladen:\n%s\n' "$OUTPUT_PATH"
