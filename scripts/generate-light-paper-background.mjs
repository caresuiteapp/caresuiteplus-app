...
function buildAnimationCss(layers) {
  const rules = layers.map((layer) => {
    const kf = `@keyframes lpb-kf-${layer.id} {
  0%, 100% { transform: translate3d(0, 0, 0) scale(1); }
  50% { transform: translate3d(${layer.dx}, ${layer.dy}, 0) scale(${layer.scalePeak}); }
}`;
    const rule = `.lpb-${layer.id} {
  animation: lpb-kf-${layer.id} ${LPB_CYCLE_S}s ease-in-out infinite;
  animation-delay: -${layer.delayS}s;
...
function writeAnimatedToken(svgInner, layers) {
  const css = buildAnimationCss(layers);
