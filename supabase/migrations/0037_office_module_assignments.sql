-- ==========================================================================
-- CareSuite+ — Migration 0037: OfficeCore Modulzuordnungen
-- client_module_assignments, employee_module_assignments, module_service_catalog,
-- module_billing_sources, module_document_visibility, module_template_assignments,
-- module_permission_profiles — preparedOnly, non-destructive, RLS.
-- ==========================================================================
...
CREATE POLICY client_module_assignments_tenant ON public.client_module_assignments
  FOR ALL
  USING (tenant_id = (SELECT tenant_id FROM public.profiles WHERE id = auth.uid()))
  WITH CHECK (tenant_id = (SELECT tenant_id FROM public.profiles WHERE id = auth.uid()));

CREATE POLICY employee_module_assignments_tenant ON public.employee_module_assignments
  FOR ALL
  USING (tenant_id = (SELECT tenant_id FROM public.profiles WHERE id = auth.uid()))
  WITH CHECK (tenant_id = (SELECT tenant_id FROM public.profiles WHERE id = auth.uid()));

CREATE POLICY module_service_catalog_tenant ON public.module_service_catalog
  FOR ALL
  USING (tenant_id = (SELECT tenant_id FROM public.profiles WHERE id = auth.uid()))
  WITH CHECK (tenant_id = (SELECT tenant_id FROM public.profiles WHERE id = auth.uid()));

CREATE POLICY module_billing_sources_tenant ON public.module_billing_sources
  FOR ALL
  USING (tenant_id = (SELECT tenant_id FROM public.profiles WHERE id = auth.uid()))
  WITH CHECK (tenant_id = (SELECT tenant_id FROM public.profiles WHERE id = auth.uid()));

CREATE POLICY module_document_visibility_tenant ON public.module_document_visibility
  FOR ALL
  USING (tenant_id = (SELECT tenant_id FROM public.profiles WHERE id = auth.uid()))
  WITH CHECK (tenant_id = (SELECT tenant_id FROM public.profiles WHERE id = auth.uid()));

CREATE POLICY module_template_assignments_tenant ON public.module_template_assignments
  FOR ALL
  USING (tenant_id = (SELECT tenant_id FROM public.profiles WHERE id = auth.uid()))
  WITH CHECK (tenant_id = (SELECT tenant_id FROM public.profiles WHERE id = auth.uid()));

CREATE POLICY module_permission_profiles_tenant ON public.module_permission_profiles
  FOR ALL
  USING (tenant_id = (SELECT tenant_id FROM public.profiles WHERE id = auth.uid()))
  WITH CHECK (tenant_id = (SELECT tenant_id FROM public.profiles WHERE id = auth.uid()));

COMMENT ON TABLE public.client_module_assignments IS 'OfficeCore Klient:innen-Modulzuordnung — preparedOnly (Content Architecture Sprint)';
COMMENT ON TABLE public.employee_module_assignments IS 'OfficeCore Mitarbeitende-Modulzuordnung — preparedOnly';
COMMENT ON TABLE public.module_service_catalog IS 'Leistungskatalog je Fachmodul — preparedOnly';
