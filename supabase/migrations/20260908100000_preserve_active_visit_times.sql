-- A status-only visit mirror must retain lifecycle times already recorded by
-- the employee. The previous guard covered completed visits only, so mirroring
-- arrival/start/pause could erase timestamps in the linked assignments row.
-- Explicit corrections remain possible: only fields empty both before and
-- after this visit update inherit the linked assignment's recorded value.
