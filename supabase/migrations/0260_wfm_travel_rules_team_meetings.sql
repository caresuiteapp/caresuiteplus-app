-- CareSuite+ WFM: produktive Fahrzeitregeln und Team-Meetings
...
CREATE POLICY wfm_travel_rules_manage ON public.workforce_travel_rules
  ...
  USING (tenant_id = public.current_tenant_id() AND (public.has_permission('time.settings.manage') OR public.is_tenant_admin()))
  WITH CHECK (tenant_id = public.current_tenant_id() AND (public.has_permission('time.settings.manage') OR public.is_tenant_admin()));
...
BEGIN
  ...
  FOR UPDATE;
  IF NOT FOUND THEN RAISE EXCEPTION 'Team-Meeting nicht gefunden.'; END IF;
  IF v_meeting.status = 'cancelled' THEN RAISE EXCEPTION 'Abgesagtes Team-Meeting kann nicht gebucht werden.'; END IF;
  ...
  IF v_meeting.counts_as_work_time THEN
    ...
    SELECT
      p_tenant_id, attendee.employee_id, event.event_type, 'office', 'office',
      event.occurred_at, v_meeting.title,
      jsonb_build_object('meeting_id', p_meeting_id), p_actor_id
    ...
    WHERE tenant_id = p_tenant_id AND meeting_id = p_meeting_id AND booked_at IS NULL;
  END IF;
