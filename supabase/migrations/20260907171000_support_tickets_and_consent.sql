...
CREATE INDEX support_tickets_tenant_updated ON public.support_tickets(tenant_id,updated_at DESC,id);
CREATE INDEX support_tickets_queue ON public.support_tickets(status,updated_at DESC,id);
...
REVOKE ALL ON public.support_tickets,public.support_messages,public.support_access_requests,public.support_audit_events FROM PUBLIC,anon,authenticated;
GRANT SELECT ON public.support_tickets,public.support_messages,public.support_access_requests,public.support_audit_events TO authenticated;
...
DECLARE v_tenant uuid; v_ticket public.support_tickets%ROWTYPE; v_name text;
BEGIN
  ...
  INSERT INTO public.support_tickets(tenant_id,created_by,client_nonce,subject,category,priority)
  ...
  IF NOT FOUND THEN
    SELECT * INTO v_ticket FROM public.support_tickets WHERE created_by=auth.uid() AND client_nonce=p_client_nonce AND tenant_id=v_tenant;
...
BEGIN
  ...
  SELECT coalesce(jsonb_agg(to_jsonb(q)),'[]') INTO v_rows FROM (
    ...
    FROM public.support_tickets t JOIN public.tenants n ON n.id=t.tenant_id LEFT JOIN public.platform_users pu ON pu.id=t.assigned_to
...
BEGIN
  ...
  UPDATE public.support_tickets SET assigned_to=v_operator,updated_at=now() WHERE id=p_ticket_id;
...
RETURNS jsonb LANGUAGE plpgsql SECURITY DEFINER SET search_path=public,pg_temp AS $$
DECLARE v_ticket public.support_tickets%ROWTYPE; v_operator public.platform_users%ROWTYPE; v_request public.support_access_requests%ROWTYPE;
BEGIN
...
RETURNS jsonb LANGUAGE plpgsql SECURITY DEFINER SET search_path=public,pg_temp AS $$
DECLARE v_request public.support_access_requests%ROWTYPE; v_ticket public.support_tickets%ROWTYPE;
BEGIN
  ...
  SELECT * INTO v_ticket FROM public.support_tickets WHERE id=v_request.ticket_id FOR UPDATE;
