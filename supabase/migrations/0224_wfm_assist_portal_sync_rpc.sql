...
-- Root cause: direct workforce_work_sessions UPDATE blocked for portal actors without
-- time.tracking.admin.correct; session/event mirror during Assist workflow is best-effort
-- client-side and silently dropped. Budget fix pattern: RPC bypasses table RLS safely.

-- Audit: service_role SELECT on sessions (0215 only covered workforce_time_events)
DO $$
BEGIN
...
-- Portal may update own session during assist mirror (not only admin.correct)
