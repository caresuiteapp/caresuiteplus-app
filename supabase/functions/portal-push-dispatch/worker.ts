...
const TITLES: Record<string, string> = {
  ...
  update: 'CareSuite-Update verfügbar',
};
export function notificationFor(
  ...
  return {
    ...
    title: TITLES[work.event_kind] ?? 'Neue CareSuite-Mitteilung',
    body: 'Öffnen Sie CareSuite, um die Informationen in Ihrem Portal anzusehen.',
    sound: 'default',
    channelId: 'caresuite-important',
    priority: 'high',
...
export async function processPortalPush(
  queue: PushQueue,
  ...
  if (pendingReceipts.length) {
    try {
      ...
      for (const item of pendingReceipts) {
        ...
        if (receipt?.status) {
          await queue.receipt(item, receipt.status, receipt.details?.error ?? null);
          summary.receipts++;
        } else if (now - Date.parse(item.updated_at) > 23 * 3600_000)
          await queue.receipt(item, 'error', 'ReceiptUnavailable');
      }
    } catch {
      /* Accepted tickets are never resent because receipt lookup failed. */
    }
  }
  ...
  for (const item of work) {
    try {
      ...
      if (!target || !(Date.parse(item.expires_at) > now)) {
        await queue.finish(item, 'cancelled', null, 'no_longer_accessible');
        summary.cancelled++;
      } else ready.push({ work: item, target });
    } catch {
      await queue.finish(item, 'retry', null, 'target_check_failed');
      summary.retry++;
    }
  }
  ...
  try {
    tickets = await transport.send(
      ready.map(({ work, target }) => notificationFor(work, target, now)),
    );
  } catch {
    for (const { work: item } of ready) {
      await queue.finish(item, 'retry', null, 'transport_unavailable');
      summary.retry++;
    }
    return summary;
  }
  for (let i = 0; i < ready.length; i++) {
    const item = ready[i].work;
    const result = tickets[i];
    if (result?.status === 'ok' && result.id) {
      await queue.finish(item, 'accepted', result.id, null);
      summary.accepted++;
      continue;
    }
    ...
    const outcome: DeliveryOutcome = [
      ...
      ? 'failed'
      : 'retry';
    await queue.finish(item, outcome, null, error);
    summary[outcome]++;
  }
  return summary;
}
