...
Deno.serve(async (req) => {
  ...
  try {
    ...
    const { data: allowed, error: authError } = await db.rpc('portal_push_worker_authorized', {
      token_hash: hash,
    });
    ...
    const queue: PushQueue = {
      ...
      async target(item) {
        const { data, error } = await db.rpc('portal_push_delivery_target', {
          outbox_id: item.id,
          claim_token: item.lease_token,
        });
        if (error) throw error;
        return data?.[0] ?? null;
      },
      async finish(item, outcome, ticket, failure) {
        const { data, error } = await db.rpc('portal_push_finish', {
          outbox_id: item.id,
          claim_token: item.lease_token,
          outcome,
          ticket,
        ...
        if (error || data !== true) throw error ?? new Error('Claim expired');
      },
      async receipts() {
        const { data, error } = await db
          .from('portal_push_outbox')
          .select('id,expo_ticket_id,updated_at')
          .eq('state', 'accepted')
      ...
      async receipt(item, status, failure) {
        const { error } = await db.rpc('portal_push_receipt', {
          outbox_id: item.id,
          ticket: item.expo_ticket_id,
    ...
    async function post(path: 'send' | 'getReceipts', body: unknown) {
      ...
      if (!payload.data) throw new Error('Invalid push response');
      return payload.data;
    }
    const transport: PushTransport = {
      send: (messages) => post('send', messages),
      receipts: (ids) => post('getReceipts', { ids }),
    };
    return jsonResponse({ ok: true, ...(await processPortalPush(queue, transport)) });
  } catch {
    return jsonResponse(
      { ok: false, error: 'Push dispatch failed; queued work remains retryable.' },
      503,
    );
  }
});
