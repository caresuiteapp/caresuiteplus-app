...
export type EmployeeTimeTrackingMode = 'field' | 'homeoffice' | 'hybrid' | 'office' | 'none';
...
export const EMPLOYEE_TIME_TRACKING_MODE_OPTIONS: Array<{
  ...
  { key: 'hybrid', label: EMPLOYEE_TIME_TRACKING_MODE_LABELS.hybrid },
  { key: 'office', label: EMPLOYEE_TIME_TRACKING_MODE_LABELS.office },
];

const timeTrackingModeStore = new Map<string, EmployeeTimeTrackingMode | null>();

function timeModeStoreKey(tenantId: string, employeeId: string): string {
  return `${tenantId}:${employeeId}:time_mode`;
}

export function getEmployeeTimeTrackingModeOverride(
  employeeId: string,
  tenantId?: string,
): EmployeeTimeTrackingMode | null {
  if (tenantId) {
    return timeTrackingModeStore.get(timeModeStoreKey(tenantId, employeeId)) ?? null;
  }
  return timeTrackingModeStore.get(employeeId) ?? null;
}

export function setEmployeeTimeTrackingModeOverride(
  employeeId: string,
  value: EmployeeTimeTrackingMode | null,
  tenantId?: string,
): void {
  const key = tenantId ? timeModeStoreKey(tenantId, employeeId) : employeeId;
  if (value === null) {
    timeTrackingModeStore.delete(key);
    return;
  }
  timeTrackingModeStore.set(key, value);
}

export async function loadEmployeeTimeTrackingModeOverride(
  tenantId: string,
  employeeId: string,
): Promise<EmployeeTimeTrackingMode | null> {
  if (getServiceMode() !== 'supabase') {
    return getEmployeeTimeTrackingModeOverride(employeeId, tenantId);
  }
  ...
  const mode = (data as { time_tracking_mode?: string | null } | null)?.time_tracking_mode;
  const parsed = mode as EmployeeTimeTrackingMode | null;
  setEmployeeTimeTrackingModeOverride(employeeId, parsed, tenantId);
  return parsed;
}

export async function persistEmployeeTimeTrackingModeOverride(
  tenantId: string,
  employeeId: string,
  value: EmployeeTimeTrackingMode | null,
): Promise<ServiceResult<void>> {
  setEmployeeTimeTrackingModeOverride(employeeId, value, tenantId);
  ...
  if (value === null) {
    const { error } = await fromUnknownTable(supabase, 'employee_work_settings')
      .update({ time_tracking_mode: null, updated_at: new Date().toISOString() })
      .eq('tenant_id', tenantId)
      .eq('employee_id', employeeId);
...
export function resolveEmployeeTimeTrackingModeWithOverride(
  roleKey: RoleKey | null,
  homeOfficeOverride: boolean | null = null,
  modeOverride: EmployeeTimeTrackingMode | null = null,
): EmployeeTimeTrackingMode {
  if (modeOverride) return modeOverride;
  return resolveEmployeeTimeTrackingMode(roleKey, homeOfficeOverride);
}
