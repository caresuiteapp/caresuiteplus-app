...
function buildDetailFromDemo(
  ...
  return {
    ...
    recipient: {
      ...
      city: '',
      country: 'Deutschland',
      customerNumber: invoice.clientId,
    },
    taxNotice: null,
    notes:
...
function buildDetailFromSupabase(row: InvoiceRow): InvoiceDetail {
  ...
  return {
    ...
    billingModule: row.billing_module,
    ...
    recipient: {
      ...
      customerNumber: row.client_number,
    },
    taxNotice: row.footer_text,
    ...
    lineItems: (row.line_items ?? []).map((item) => ({
      ...
      netTotalCents: Math.round(Number(item.net_amount ?? item.gross_amount ?? 0) * 100),
      taxRatePercent: Number(item.vat_rate ?? 0),
      taxCents: Math.round(Number(item.vat_amount ?? 0) * 100),
