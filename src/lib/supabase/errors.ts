...
export function toGermanSupabaseError(error: unknown): string {
  if (!error || typeof error !== 'object') return 'Ein unerwarteter Datenbankfehler ist aufgetreten.';
  ...
  if (code === 'PGRST116') return 'Datensatz wurde nicht gefunden.';
  ...
  if (code === 'PGRST204' || msg.includes('Could not find the') && msg.includes('column')) {
    return isDevEnvironment()
  ...
  if (isDevEnvironment() && msg) {
    return `${GENERIC_DB_ERROR} [${code ?? 'unknown'}: ${msg}]`;
  }
