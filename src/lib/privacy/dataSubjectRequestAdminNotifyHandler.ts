import { DATA_REQUEST_TYPE_LABELS } from './dataSubjectRequestLabels';
import type { DataRequestType } from './dataSubjectRequest.types';

export const DSGVO_ADMIN_NOTIFY_EDGE_FUNCTION = 'notify-data-subject-request-admin';
...
export function buildAdminNotifyEmailContent(request: AdminNotifyRequestPayload): {
  ...
  const typeLabel = DATA_REQUEST_TYPE_LABELS[request.requestType] ?? request.requestType;
