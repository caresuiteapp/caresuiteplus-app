...
export const SUPPORT_STATUS: Record<string, string> = { open: 'Offen', waiting_tenant: 'Antwort des Unternehmens ausstehend', waiting_support: 'Antwort des Supports ausstehend', resolved: 'Gelöst', closed: 'Geschlossen' };
...
export async function pickSupportAttachment(ticketId: string): Promise<SupportAttachment | null> {
  ...
  try {
    ...
    return await supportRpc<SupportAttachment>('support_finalize_attachment', { p_attachment_id: attachment.id });
  } catch (cause) {
    // The file has not been attached to a message yet. Release the reserved slot
    // so unsuccessful attempts do not exhaust the upload quota.
    await removeSupportDraftAttachment(attachment).catch(() => undefined);
    throw cause;
  }
}
