...
function assessment(row: Row, children: Row[][]): CareAssessment {
  const [topicRows, riskRows, measureRows, evaluationRows, events, links, versionRows] = children;
  ...
  return {
    ...
    evaluations: evaluationRows.map((entry) => ({
      id: String(entry.id ?? ''),
      outcome: String(entry.outcome ?? 'not_assessable') as CareAssessmentEvaluation['outcome'],
      observedEffect: String(entry.observed_effect ?? ''),
      personFeedback: String(entry.person_feedback ?? ''),
      professionalConclusion: String(entry.professional_conclusion ?? ''),
      changesRequired: String(entry.changes_required ?? ''),
      evaluatedAt: String(entry.evaluated_at ?? ''),
      evaluatorName: String(entry.evaluator_name_snapshot ?? ''),
    })),
    events,
    links,
    versionHistory: versionRows.map((entry) => ({
      id: String(entry.id ?? ''),
      version: Number(entry.version ?? 1),
      transitionFrom: entry.transition_from ? String(entry.transition_from) : null,
      transitionTo: String(entry.transition_to ?? ''),
...
export const careAssessmentRepository = {
  async listEligibleCareClients(tenantId: string): Promise<ServiceResult<CareAssessmentSubjectOption[]>> {
    const supabase = getSupabaseClient();
    if (!supabase) return unavailable();
    const rpc = supabase as unknown as {
      rpc: (name: string) => Promise<{ data: Row[] | null; error: Parameters<typeof toGermanSupabaseError>[0] }>;
    };
    const { data, error } = await rpc.rpc('list_pfleger_clients');
    if (error) return { ok: false, error: toGermanSupabaseError(error) };
    return {
      ok: true,
      data: (data ?? [])
        .filter((row) => String(row.tenant_id ?? tenantId) === tenantId)
        .map((row) => ({
          id: String(row.id),
          firstName: String(row.first_name ?? ''),
          lastName: String(row.last_name ?? ''),
          dateOfBirth: row.date_of_birth ? String(row.date_of_birth) : null,
          careLevel: row.care_level ? String(row.care_level) : null,
        })),
    };
  },
  ...
  async addEvaluation(
    ...
    const supabase = getSupabaseClient();
    if (!supabase) return unavailable();
    const { error } = await fromUnknownTable(supabase, 'care_assessment_evaluations').insert({
      tenant_id: tenantId,
      assessment_id: assessmentId,
      outcome: evaluation.outcome,
      observed_effect: evaluation.observedEffect,
      person_feedback: evaluation.personFeedback,
      professional_conclusion: evaluation.professionalConclusion,
      changes_required: evaluation.changesRequired,
      evaluated_at: evaluation.evaluatedAt,
      evaluator_name_snapshot: evaluation.evaluatorName,
    });
    if (error) return { ok: false, error: toGermanSupabaseError(error) };
    return this.get(tenantId, assessmentId);
  },
};
