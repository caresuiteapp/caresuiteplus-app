...
const key = (kind: RememberedPortalKind, part: string) =>
  `caresuite.remembered-login.v1.${kind}.${part}`;
export const supportsRememberedPortalLogin = () =>
  Platform.OS === 'android' || Platform.OS === 'ios';

export async function getRememberedPortalMetadata(
  kind: RememberedPortalKind,
): Promise<RememberedPortalMetadata | null> {
  ...
  if (
    ...
    throw new Error('Gespeicherte Anmeldung ist unvollständig. Bitte normal anmelden.');
  return {
    version: 1,
    kind,
...
export async function saveRememberedPortalLogin(item: RememberedPortalLogin): Promise<void> {
  if (!supportsRememberedPortalLogin())
    throw new Error('Speichern ist nur in der installierten App möglich.');
  if (!item.accountId || !item.tenantId || !item.username.trim() || !item.secret)
    throw new Error('Anmeldung ist unvollständig.');
...
export async function unlockRememberedPortalLogin(
  kind: RememberedPortalKind,
): Promise<RememberedPortalLogin> {
  ...
  if (
    ...
    item.accountId !== after.accountId ||
    item.tenantId !== after.tenantId ||
    item.kind !== kind ||
    item.username !== after.username ||
    !item.secret
  )
    throw new Error('Die gespeicherte Anmeldung hat sich geändert. Bitte normal anmelden.');
  return item;
}
export async function forgetRememberedPortalLogin(
  kind: RememberedPortalKind,
  accountId?: string,
): Promise<void> {
