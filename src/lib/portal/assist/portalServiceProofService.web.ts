...
function resolveProofStatus(row: Record<string, unknown>): PortalServiceProofStatus {
  ...
  if (signedAt || ['signed', 'approved', 'billable'].includes(billingStatus)) return 'unterschrieben';
...
export async function listPortalServiceProofs(
  ...
  return runService(async () => {
    ...
    const serviceRecordResult = await fetchAllPortalRows<Record<string, unknown> & { id: string }>((from, to) => fromUnknownTable(supabase, 'service_records')
      .select('id, tenant_id, client_id, record_number, service_date, service_month, status, pdf_url, created_at', { count: 'exact' })
      .eq('tenant_id', tenantId).eq('client_id', clientId)
      .in('status', ['signature_required', 'signed', 'approved', 'billable', 'billed'])
      .order('service_date', { ascending: false }).order('id', { ascending: true }).range(from, to));
    if (!serviceRecordResult.ok) return serviceRecordResult;
    ...
    const proofs = (serviceRecordResult.data ?? []).map((row) => {
      ...
      return mapProofRow({
        ...
        signature_required: record.status === 'signature_required',
    ...
    return appendAssistVisitProofs(tenantId, clientId, proofs);
  });
}
...
async function appendAssistVisitProofs(
  ...
  if (!assistProofs.ok) return assistProofs;
  ...
  for (const proof of assistMapped) byId.set(proof.id, proof);
  return { ok: true, data: [...byId.values()].sort((a, b) => Number(b.signatureRequired) - Number(a.signatureRequired) || b.createdAt.localeCompare(a.createdAt)) };
}
