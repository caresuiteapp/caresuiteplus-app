...
export async function uploadEmployeePortalDocument(
  ...
  return runService(async () => {
    ...
    const uploadId = globalThis.crypto?.randomUUID?.() ?? 'xxxxxxxx-xxxx-4xxx-yxxx-xxxxxxxxxxxx'.replace(/[xy]/g, token => {
      const byte = globalThis.crypto.getRandomValues(new Uint8Array(1))[0] & 15;
      return (token === 'x' ? byte : (byte & 3) | 8).toString(16);
    });
    ...
    if (input.uploadContext === 'klient' && input.clientId?.trim()) {
      ...
      if (!requestResult.ok) {
        return { ok: false, error: requestResult.error ?? 'Die Weiterleitung an das Office konnte nicht bestätigt werden.' };
      }
    ...
    try {
      const persisted = await fromUnknownTable(supabase, 'portal_uploads')
        .insert({
          id: uploadId,
        ...
        .select('*')
        .single();
      data = persisted.data as Record<string, unknown> | null;
      error = persisted.error;
    } catch (cause) { error = cause; }

    if (error || !data) {
      // A lost HTTP response does not mean the insert was rolled back. Never delete its file.
      const controller = new AbortController();
      const timer = setTimeout(() => controller.abort(), 6000);
      try {
        const recovered = await fromUnknownTable(supabase, 'portal_uploads').select('*')
          .eq('id', uploadId).eq('tenant_id', input.tenantId).eq('employee_id', input.employeeId)
          .eq('storage_path', storagePath).abortSignal(controller.signal).maybeSingle();
        if (!recovered.error && recovered.data) return { ok: true, data: mapUploadRow(recovered.data as Record<string, unknown>) };
      } catch { /* Confirmation is still uncertain; keep the uploaded object. */ }
      finally { clearTimeout(timer); }

      if (isMissingTableError(error)) {
        return { ok: false, error: 'Uploads sind noch nicht verfügbar (Migration ausstehend).' };
      }
      return { ok: false, error: `${error ? toGermanSupabaseError(error) : 'Keine Speicherbestätigung erhalten.'} Bitte die Upload-Liste aktualisieren, bevor Sie die Datei erneut einreichen.` };
    }
