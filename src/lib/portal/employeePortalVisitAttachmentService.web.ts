...
function mapAttachmentRow(row: Record<string, unknown>): EmployeePortalVisitAttachment {
  ...
  return {
    ...
    mediaKind: ['image', 'video', 'audio', 'document'].includes(String(row.media_kind))
      ? row.media_kind as EmployeePortalVisitAttachment['mediaKind'] : resolveMediaKind(mimeType),
...
export async function uploadEmployeePortalVisitAttachment(
  ...
  if (!input.tenantId?.trim() || input.bytes.length === 0) {
    return { ok: false, error: 'Dateiinhalt oder Unternehmenszuordnung fehlt.' };
  }
  ...
  const audioTypes = new Set(['audio/webm', 'audio/ogg', 'audio/mpeg', 'audio/mp3', 'audio/wav', 'audio/mp4', 'audio/x-m4a', 'audio/m4a']);
  if (audioTypes.has(mimeType)) {
    if (input.bytes.length > 50 * 1024 * 1024) return { ok: false, error: 'Die Audio-Aufnahme ist größer als 50 MB. Bitte eine kürzere Aufnahme verwenden.' };
  } else {
