...
export async function fetchPortalCsDocumentRequests(input: {
  ...
  try {
    ...
    const sigByRequest = new Map<string, CsDocumentRequestSignature[]>();
    const pageSize = 100;
    for (let offset = 0, expected: number | null = null; ;) {
    let query = fromUnknownTable(supabase, 'cs_document_requests')
      .select(input.summaryOnly ? 'id, owner_tenant_id, title, recipient_scope, client_id, employee_id, representative_id, status, due_date, portal_visible, priority, created_at, updated_at, completed_at' : '*', { count: 'exact' })
      .eq('owner_tenant_id', tenantId)
    ...
    if (error) throw error;
    if (count == null || (expected !== null && expected !== count)) throw new Error('Dokumentliste wurde geändert. Bitte neu laden.');
    expected = count;
    ...
    if (ids.length === 0) {
      if (requests.length !== expected) throw new Error('Dokumentliste ist unvollständig.');
      break;
    }

    const signaturePage = await fetchAllPortalRows((from, to) => fromUnknownTable(supabase, 'cs_document_request_signatures')
      .select('id, request_id, signer_role, signer_name, status, signed_at', { count: 'exact' })
      .in('request_id', ids).order('id', { ascending: true }).range(from, to));
    if (!signaturePage.ok) throw new Error(signaturePage.error);
    const sigData = signaturePage.data;
    ...
    for (const row of sigData ?? []) {
      ...
      if (offset > expected) throw new Error('Dokumentliste wurde geändert.');
    }
...
export async function signCsDocumentRequest(input: {
  ...
  try {
    ...
    if (input.portalActor?.roleKey === 'client_portal' || input.portalActor?.roleKey === 'family_portal') {
      const receipt = await supabase.rpc('client_portal_sign_document_request' as never, {
        p_tenant_id: input.tenantId, p_request_id: input.requestId,
        p_signer_role: input.signerRole, p_signer_name: input.signerName.trim(),
        p_signature_data_url: input.signatureDataUrl,
        p_expected_updated_at: detail.data.updatedAt,
      } as never);
      if (receipt.error) throw receipt.error;
      if (receipt.data !== input.requestId) throw new Error('Unterschrift wurde nicht bestätigt. Bitte den Status erneut laden.');
    ...
    const { data: remaining, error: remainingError } = await fromUnknownTable(supabase, 'cs_document_request_signatures')
      .select('id')
      .eq('request_id', input.requestId)
      .eq('status', 'pending');
