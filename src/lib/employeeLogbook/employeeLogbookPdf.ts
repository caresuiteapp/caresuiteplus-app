...
export async function buildLogbookPdf(input: { employeeName: string; from: string; to: string; trips: LogbookTrip[]; vehicles: LogbookVehicle[]; segments?: LogbookSegment[]; receipts?: LogbookReceipt[]; confirmations?: LogbookDailyConfirmation[] }) {
  ...
  pdf.setFont(CARESUITE_PDF_FONT, 'bold'); pdf.setFontSize(18); writePdfText('CareSuite HealthOS · Fahrtenbuch', 14, 18);
  pdf.setFont(CARESUITE_PDF_FONT, 'normal'); pdf.setFontSize(10); writePdfText(`${input.employeeName} · ${input.from} bis ${input.to}`, 14, 25);
  const maxWidth = 182;
  let y = 35;
  ...
  const write = (value: string, options?: { bold?: boolean; size?: number; color?: [number, number, number] }) => {
    ...
    pdf.setFont(CARESUITE_PDF_FONT, options?.bold ? 'bold' : 'normal');
  ...
  for (let page = 1; page <= pages; page += 1) {
    pdf.setPage(page);
    pdf.setFont(CARESUITE_PDF_FONT, 'normal'); pdf.setFontSize(8); pdf.setTextColor(75, 97, 122);
    writePdfText(`Seite ${page} von ${pages}`, 196, 290, { align: 'right' });
  }
