...
export { isAssistExecutionPersistenceReady };

export async function fetchValidVisitSignature(tenantId: string, visitId: string): Promise<ServiceResult<AssistVisitSignatureRow | null>> {
  ...
  try {
    const { data, error } = await fromUnknownTable(supabase, ASSIST_EXECUTION_TABLES.signatures)
      .select('*').eq('tenant_id', tenantId).eq('visit_id', visitId).eq('is_valid', true)
      .order('signed_at', { ascending: false }).order('created_at', { ascending: false }).limit(1)
      .abortSignal(controller.signal).maybeSingle();
    if (error) return { ok: false, error: toGermanSupabaseError(error) };
    return { ok: true, data: data ? mapSignatureRow(data as SignatureDbRow) : null };
  } catch { return { ok: false, error: 'Der gespeicherte Unterschriftsstatus ist momentan nicht erreichbar. Bitte erneut laden; eine vorhandene Unterschrift bleibt erhalten.' }; }
...
export async function saveVisitSignaturePersistent(
  ...
  let { data, error } = await fromUnknownTable(supabase, ASSIST_EXECUTION_TABLES.signatures)
    .insert({
      id: signatureId,
      tenant_id: tenantId,
    ...
    .select('*')
    .single()
    .then(result => result, () => ({ data: null, error: { code: 'NETWORK', message: 'Die Serverantwort ist ausgeblieben.' } }));

  if (error) {
    // A lost response does not prove the INSERT failed. Never delete signature
    // bytes that a committed row may already reference.
    const controller = new AbortController();
    const timer = setTimeout(() => controller.abort(), 6000);
    try {
      const recovered = await fromUnknownTable(supabase, ASSIST_EXECUTION_TABLES.signatures)
        .select('*').eq('tenant_id', tenantId).eq('visit_id', input.visitId).eq('id', signatureId)
        .abortSignal(controller.signal).maybeSingle();
      const row = recovered.data as SignatureDbRow | null;
      if (!recovered.error && row?.is_valid && row.payload_hash === input.payloadHash && row.signature_hash === input.signatureHash && row.storage_path === storagePath) {
        data = row; error = null;
      }
    } catch { /* The capture remains available for an explicit retry. */ }
    finally { clearTimeout(timer); }
  }

  if (error || !data) {
    if (!error) return { ok: false, error: 'Die Speicherung ist noch nicht bestätigt. Bitte den Serverstatus erneut laden. Die Unterschrift wird nicht automatisch gelöscht.' };
