...
describe('3D-Bodymap visuelle Vergleichsmatrix', () => {
  it('enthält genau 24 Grundmodelle und sechs zusätzliche Divers-Varianten', () => {
    expect(BODY_MAP_VISUAL_QA_CASES).toHaveLength(30);
    ...
    expect(new Set(BODY_MAP_VISUAL_QA_CASES.map((entry) => entry.id)).size).toBe(30);
  });
