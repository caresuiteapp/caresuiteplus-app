...
export function useAssistCalendarEvents(rangeStart?: string, rangeEnd?: string) {
  const { profile } = useAuth();
  const tenantId = useServiceTenantId();
  const { settings } = useTenantCalendarSettings('assist');

  const query = useAsyncQuery(
    () => {
      if (!tenantId) return Promise.resolve({ ok: false as const, error: 'Kein Mandant.' });
      return fetchAssistCalendarEvents(tenantId, profile?.roleKey, { rangeStart, rangeEnd });
    },
