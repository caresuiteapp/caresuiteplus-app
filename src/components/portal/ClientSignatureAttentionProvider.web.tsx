...
import {
  fetchClientPendingDocuments,
  fetchClientPendingProofs,
  type ClientSignatureItem,
} from '@/lib/portal/clientSignatureAttention';

import { subscribePortalProofCache } from '@/lib/portal/portalProofCacheSignal';

const AttentionContext = createContext({
  items: [] as ClientSignatureItem[],
  loading: false,
  ...
  const live = useMemo(
    () => ({
      ...
      subscribe: (tid: string, handler: () => void) =>
        subscribeToClientPortalDocumentRequestChanges(tid, clientId ?? '', handler),
      pollMs: 30_000,
    }),
    [tenantId, clientId],
  );
  const proofs = useAsyncQuery(
    () => fetchClientPendingProofs(tenantId ?? '', clientId ?? '', roleKey),
    [key],
    { enabled, queryKey: key, live },
  );
  const documents = useAsyncQuery(
    () => fetchClientPendingDocuments(tenantId ?? '', clientId ?? '', roleKey),
    [key],
    { enabled, queryKey: key, live },
  );
  const refreshProofs = proofs.refresh;
  const refreshDocuments = documents.refresh;
  useEffect(() => subscribePortalProofCache(() => {
    if (enabled) void Promise.all([refreshProofs(), refreshDocuments()]);
  }), [enabled, refreshProofs, refreshDocuments]);
  const value = useMemo(
    () => ({
      ...
      refresh: async () => {
        await Promise.all([refreshProofs(), refreshDocuments()]);
      },
    }),
    [
      proofs.data,
