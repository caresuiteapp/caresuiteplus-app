...
type ProtocolDetailHeroProps = {
  protocol: Protocol & { caseSubject: string };
  roleKey: RoleKey;
};

export function ProtocolDetailHero({ protocol, roleKey }: ProtocolDetailHeroProps) {
  ...
  const kpis = buildProtocolDetailKpis(protocol, mode);

  return (
    <PremiumListHeroFrame>
      <View style={styles.topRow}>
        <View style={styles.textCol}>
          <Text style={styles.eyebrow}>BERATUNG · PROTOKOLL</Text>
          <Text style={styles.title}>{protocol.caseSubject}</Text>
          <Text style={styles.meta}>
            {new Date(protocol.recordedAt).toLocaleDateString('de-DE')} · Fall {protocol.caseId}
          </Text>
        </View>
