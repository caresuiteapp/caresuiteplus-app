...
function createModuleOverviewStyles(isPhone: boolean) {
  return StyleSheet.create({
    container: {
      width: '100%',
      minHeight: isPhone ? undefined : MODULE_OVERVIEW_MIN_GRID_HEIGHT,
    },
    gridRow: {
...
/** Module columns with KPI stacks — single column scroll on phone, multi-column on tablet+. */
export function ModuleOverviewDashboard({ rows }: ModuleOverviewDashboardProps) {
  const router = useRouter();
  const { isPhone } = useDeviceClass();
  const styles = useMemo(() => createModuleOverviewStyles(isPhone), [isPhone]);
