...
import type { CalendarEvent, CalendarModuleScope, CalendarViewMode } from '@/types/modules/calendarEvent';
import { EmptyState, ErrorState, LoadingState } from '@/components/ui';
import { careSpacing } from '@/design/tokens/spacing';
import { useAssistCalendarEvents } from '@/hooks/useAssistCalendarEvents';
...
function useScopedCalendar(
  scope: CalendarModuleScope,
  rangeStart: string,
  rangeEnd: string,
) {
  const office = useOfficeCalendar(rangeStart, rangeEnd);
  const assist = useAssistCalendarEvents(rangeStart, rangeEnd);
  return scope === 'assist' ? assist : office;
}

export function ModuleCalendarView({ scope = 'office', onEventPress }: ModuleCalendarViewProps) {
  const { settings, form, saving, save, loading: settingsLoading } = useTenantCalendarSettings(scope);
  ...
  const { events, loading, error, refresh } = useScopedCalendar(
    scope,
    range.rangeStart,
    range.rangeEnd,
  );
    ...
    <View style={styles.wrap}>
      ...
      {events.length === 0 ? (
        <EmptyState
          ...
          message={
            scope === 'assist'
              ? 'Für diesen Zeitraum sind keine Assist-Einträge sichtbar.'
              : 'Für diesen Zeitraum sind keine Kalendereinträge sichtbar.'
          }
        />
      ) : null}
