...
export function VisitProofReviewPanel({
  ...
  const runAction = async (action: () => Promise<{ ok: boolean; error?: string }>) => {
    ...
    try {
      ...
      // Multi-step actions may persist the proof before document delivery fails.
      // Reconcile the list even on an error; keep the error visible in this panel.
      onUpdated?.();
    } catch (cause) {
      setError(cause instanceof Error ? cause.message : 'Aktion fehlgeschlagen. Bitte den aktuellen Stand prüfen.');
      onUpdated?.();
    } finally {
  ...
  return (
    ...
    >
      {busy ? <InfoBanner variant="info" title="Nachweis wird verarbeitet" message="Bitte warten. Die Aktion läuft weiter; ein erneuter Klick ist gesperrt." /> : null}
