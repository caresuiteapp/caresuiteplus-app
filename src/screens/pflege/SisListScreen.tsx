...
/** Arbeitsplan 050 — /pflege/sis */
export function SisListScreen() {
  ...
  const openDetail = (id: string) => router.push(`/pflege/sis/${id}` as never);

  if (query.loading && items.length === 0) {
    return (
      <CareLightPageShell title="SIS Liste" subtitle="Wird geladen…">
  ...
  if (query.error && items.length === 0) {
    return (
      <CareLightPageShell title="SIS Liste" subtitle="Fehler">
  ...
  const shellProps = {
    title: 'SIS Liste' as const,
    subtitle: `Strukturierte Informationssammlung · ${roleLabel ?? 'Demo'}`,
    rightSlot: !isReadOnly ? (
      <PremiumButton title="+ Neu" size="sm" onPress={() => router.push('/pflege/sis/new' as never)} />
    ) : undefined,
    ...
    return (
      <CareLightPageShell {...shellProps}>
        ...
        >
          {header}
          {items.length === 0 ? (
            <EmptyState title="Keine SIS-Einträge" message="Es sind noch keine SIS-Assessments angelegt." />
          ) : (
