...
export function ClientDocumentSignaturesScreen() {
  ...
  const items =
    filter === 'open'
      ...
      : (query.data ?? []).filter((r) => !r.pendingSignatureRoles.some((role) => role === 'client' || role === 'representative') && r.signatures.some((signature) => ['client', 'representative'].includes(signature.signerRole) && signature.status === 'signed'));
  ...
  return (
    ...
    >
      <ScrollView contentContainerStyle={styles.scroll} showsVerticalScrollIndicator>
        ...
        {(query.error || query.refreshError) ? (
          <ErrorState
            message={toPortalUserFacingError(query.error || query.refreshError, 'Ihre Dokumente konnten gerade nicht geladen werden. Bitte versuchen Sie es erneut.')}
            onRetry={query.refresh}
          />
        ) : null}
        {items.length === 0 && (filter === 'done' || proofs.length === 0) && !attention.loading && !attention.error && !query.loading && !query.error && !query.refreshError ? (
          <ClientPortalGuide
            compact
