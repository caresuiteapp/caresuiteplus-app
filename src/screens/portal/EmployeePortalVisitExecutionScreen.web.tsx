import type { EmployeePortalVisitDocumentationPanelHandle } from '@/components/portal/EmployeePortalVisitDocumentationPanel.web';
import { webScaledFontMetric as font } from '@/design/web/webFontSize';
import { EmployeePortalVisitFabMenu } from '@/components/portal/EmployeePortalVisitFabMenu.web';
import { EmployeePortalVisitDocumentationPanel } from '@/components/portal/EmployeePortalVisitDocumentationPanel.web';
import { EmployeePortalVisitCompletionPanel } from '@/components/portal/EmployeePortalVisitCompletionPanel.web';
import { EmployeePortalVisitBottomBar } from '@/components/portal/EmployeePortalVisitBottomBar.web';
...
export function EmployeePortalVisitExecutionScreen() {
  ...
  const [signatureConfirmationStalled, setSignatureConfirmationStalled] = useState(false);
  ...
  useEffect(() => {
    ...
    void workflowPersistence.restoreAsync().then((snapshot) => {
      ...
      if (!documentationEditedRef.current && snapshotMatchesRoute && typeof snapshot?.documentationDraftText === 'string') {
        ...
        if (snapshot.documentationDraftText.trim()) setDocumentationDirty(true);
      }
  ...
  useEffect(() => {
    ...
    if (signatureSaveError) {
      setSignatureConfirmationPending(false);
      setSignatureConfirmationStalled(false);
      setLocalError(signatureSaveError);
      return;
    }
    if (signatureConfirmationStalled) return;
    return pollSignatureConfirmation({
      refresh: () => signatureConfirmationRefreshRef.current(),
      onUnconfirmed: () => setSignatureConfirmationStalled(true),
    });
  }, [
  ...
  useEffect(() => {
    ...
    workflowPersistence.persist({
      ...
      showNoShowForm,
      documentationSubmitted,
      documentationDraftText: documentationSubmitted && !documentationDirty ? null : documentationDraftText,
      documentationSpecialNotes: documentationSubmitted && !documentationDirty ? null : documentationSpecialNotes,
      documentationDeviations: documentationSubmitted && !documentationDirty ? null : documentationDeviations,
      documentationDeviationJustification: documentationSubmitted && !documentationDirty
        ? null
  ...
  const actualServiceStart = visit?.actualStartAt ? Date.parse(visit.actualStartAt) : NaN;
  const actualServiceEnd = visit?.actualEndAt ? Date.parse(visit.actualEndAt) : NaN;
  const serviceTimeComplete = Number.isFinite(actualServiceStart) && Number.isFinite(actualServiceEnd) && actualServiceEnd >= actualServiceStart;
  const serviceDurationLabel = serviceTimeComplete && visit?.actualStartAt && visit?.actualEndAt
  ...
  const syncWarning = !queryError && (!signatureConfirmationPending || signatureConfirmationStalled) && phase !== 'completed'
    ...
    if (signatureConfirmationPending) {
      if (signatureConfirmationStalled) {
        return {
          ...
          message: 'Die Speicherung der Unterschrift ist noch nicht bestätigt. Prüfe den Serverstatus. Eine offene Erfassung bitte geöffnet lassen; die Unterschrift ist noch nicht als gespeichert bestätigt.',
        };
      }
      return {
        tone: 'info' as const,
      ...
      return (
        <View style={styles.liveWrap}>
          ...
          <EmployeePortalVisitLiveDashboard
            ...
            signatureCaptured={signatureCaptured && !signatureConfirmationPending}
            signatureConfirmationPending={signatureConfirmationPending}
            signatureConfirmationStalled={signatureConfirmationStalled}
            onCheckSignature={retrySignatureConfirmation}
          ...
          {(showFinalize || canFinalizeDeferred || signatureApprovalPending) && !isLocked ? (
            <EmployeePortalVisitCompletionPanel
              tasks={visitTasks}
              documentationSubmitted={documentationSubmitted && !documentationDirty}
              ...
              serviceTimeComplete={serviceTimeComplete}
              signatureConfirmationPending={signatureConfirmationPending}
              loading={actionLoading}
              ...
              onFinalizeDeferred={async (reason) => {
                await handleFinalizeDeferredSignature(reason);
              }}
            />
          ) : null}
        </View>
    ...
    >
      <View style={styles.focusRoot} testID="employee-visit-fullscreen-workspace">
        <EmployeePortalVisitStickyHeader
          ...
          documentationComplete={documentationSubmitted && !documentationDirty}
          ...
          {(showSignature || (isServiceEnded && documentationSubmitted && signatureCaptured)) && !isLocked ? (
            ...
            >
              <EmployeePortalVisitSignaturePanel
                clientName={visit.clientName}
                loading={actionLoading}
                confirmationPending={signatureConfirmationPending}
                confirmationStalled={signatureConfirmationStalled}
                saveError={signatureSaveError}
                onCheckStatus={retrySignatureConfirmation}
                ...
                onCapture={async (sig) => {
                  ...
                  if (r.ok) {
                    ...
                    setAwaitingSignature(true);
                    setCloseSignatureCaptureRequest((n) => n + 1);
                    setLocalSuccess('Unterschrift übertragen — abschließende Bestätigung läuft.');
                    workflowPersistence.persist({
                      signatureConfirmationPending: true,
                      awaitingSignature: true,
                    });
                    ...
                    return {
                      ...r,
                      ok: false as const,
                      error: 'Die Speicherung ist noch nicht bestätigt. Lass dieses Unterschriftsfenster geöffnet; die Erfassung bleibt darin erhalten.',
                    };
                  } else {
                    setSignatureConfirmationPending(false);
                    setSignatureConfirmationStalled(false);
      ...
      {documentationAccessible ? (
        <EmployeePortalVisitDocumentationPanel
          ...
          initialDraftUnsaved={documentationDirty}
          onDirtyChange={setDocumentationDirty}
          ...
          onSubmit={async (doc) => {
            ...
            if (r.ok) {
              ...
              documentationEditedRef.current = false;
...
const styles = StyleSheet.create({
  ...
  phaseTitle: { fontSize: font(24), lineHeight: font(30), fontWeight: '800', color: portalPremium.text.primary },
  phaseHint: { fontSize: font(16), lineHeight: font(24), color: portalPremium.text.secondary },
