...
export function CsDocumentRequestDetailScreen({
  ...
  const query = useAsyncQuery(
    ...
    { enabled: !!tenantId && !!id && (!isPortal || portalActor.isLinkedReady), queryKey: JSON.stringify([tenantId, id, mode, portalActor.actorId, portalActor.clientId]) },
  );

  const item = query.data;
  const effectiveSignerRole = mode === 'client' && !item?.pendingSignatureRoles.includes('client') && item?.pendingSignatureRoles.includes('representative') ? 'representative' : signerRole;
  const canSign =
    item
    && item.pendingSignatureRoles.includes(effectiveSignerRole)
  ...
  const handleSign = async (dataUrl: string) => {
    ...
    const result = await signCsDocumentRequest({
      ...
      anchorToken: `${effectiveSignerRole}_signature`,
    ...
    <>
      ...
      >
        ...
        {item.pendingSignatureRoles.length > 0 ? (
          <PremiumBadge
            label={isPortal ? (canSign ? 'Ihre Unterschrift fehlt noch' : 'Weitere Unterschriften stehen aus') : `Offene Signatur: ${item.pendingSignatureRoles.join(', ')}`}
            variant="orange"
          />
        ) : null}
        {signSuccess ? (
        ...
        {isPortal && (signSuccess || item.status === 'completed' || item.status === 'archived') ? (
          <Pressable onPress={() => mode === 'client' ? router.replace('/portal/client/documents/signatures' as never) : router.back()} style={styles.backLink} accessibilityRole="button">
