...
async function persistServiceStartEvent(
  ...
  const recordedStart = ctx.visitTimes?.serviceStartedAt ?? ctx.detail.actualStartAt;
  const occurredAt = recordedStart && Number.isFinite(Date.parse(recordedStart)) ? recordedStart : new Date().toISOString();
...
async function backfillServiceStart(
  ...
  const recordedStart = ctx.visitTimes?.serviceStartedAt ?? ctx.detail.actualStartAt;
  if (!recordedStart || !Number.isFinite(Date.parse(recordedStart))) {
    return { ok: false, error: 'Für diesen bereits gestarteten Einsatz fehlt die tatsächliche Startzeit. Bitte unter „Zeiten nachtragen oder bearbeiten“ die tatsächliche Uhrzeit eintragen. Die geplante Zeit wird nicht automatisch übernommen.' };
  }
